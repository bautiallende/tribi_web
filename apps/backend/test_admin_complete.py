...
pytest.skip(
    "test_admin_complete exercises a running admin server via HTTP and is intended as a manual smoke script, so it's skipped in automated pytest runs.",
...
response = session.post(
    f"{BASE_URL}/api/auth/request-code", json={"email": ADMIN_EMAIL}
)
print_test(
...
print(
    f"{YELLOW}⚠ Nota: Para pruebas completas, necesitas verificar el código del email{RESET}\n"
)
...
if response.status_code in [401, 403]:
    print(
        f"{YELLOW}⚠ Admin authentication required. Skipping authenticated tests.{RESET}"
    )
    print(
        f"{YELLOW}  To run full tests, complete email verification for {ADMIN_EMAIL}{RESET}\n"
    )
    exit(0)
...
response = session.get(
    f"{BASE_URL}/admin/countries?sort_by=iso2&sort_order=asc&page_size=5"
)
countries_asc = response.json()["countries"]
print_test(
    "Sort countries by ISO2 (asc)",
...
# 2.6 Actualizar país
if test_country_id:
    update_data = {"iso2": "ZZ", "name": "Test Country Updated"}
    response = session.put(
        f"{BASE_URL}/admin/countries/{test_country_id}", json=update_data
    )
    print_test(
        "Update country", response.status_code == 200, f"Status: {response.status_code}"
    )
...
if test_country_id:
    ...
    print_test(
        "Delete country", response.status_code == 200, f"Status: {response.status_code}"
    )
...
print_test(
    "List carriers", response.status_code == 200, f"Status: {response.status_code}"
)
...
response = session.get(
    f"{BASE_URL}/admin/carriers?sort_by=name&sort_order=asc&page_size=5"
)
carriers_asc = response.json()["carriers"]
print_test(
    "Sort carriers by name (asc)",
    response.status_code == 200,
...
response = session.get(
    f"{BASE_URL}/admin/carriers?sort_by=id&sort_order=desc&page_size=5"
)
carriers_desc = response.json()["carriers"]
print_test(
    "Sort carriers by ID (desc)",
    response.status_code == 200,
    f"First ID: {carriers_desc[0]['id'] if carriers_desc else 'N/A'}",
)

# 3.5 Crear carrier
test_carrier = {"name": "Test Carrier Delete Me"}
response = session.post(f"{BASE_URL}/admin/carriers", json=test_carrier)
print_test(
    "Create carrier", response.status_code == 200, f"Status: {response.status_code}"
)
...
if test_carrier_id:
    update_data = {"name": "Test Carrier Updated"}
    response = session.put(
        f"{BASE_URL}/admin/carriers/{test_carrier_id}", json=update_data
    )
    print_test(
        "Update carrier", response.status_code == 200, f"Status: {response.status_code}"
    )
...
if test_carrier_id:
    ...
    print_test(
        "Delete carrier", response.status_code == 200, f"Status: {response.status_code}"
    )
...
response = session.get(
    f"{BASE_URL}/admin/plans?sort_by=price_usd&sort_order=asc&page_size=5"
)
...
response = session.get(
    f"{BASE_URL}/admin/plans?sort_by=duration_days&sort_order=desc&page_size=5"
)
plans_by_duration = response.json()["plans"]
print_test(
    "Sort plans by duration (desc)",
    response.status_code == 200,
...
response = session.get(
    f"{BASE_URL}/admin/plans?sort_by=data_gb&sort_order=asc&page_size=5"
)
plans_by_data = response.json()["plans"]
print_test(
    "Sort plans by data (asc)",
    response.status_code == 200,
...
if countries_data["countries"] and carriers_data["carriers"]:
    ...
    print_test(
        "Create plan", response.status_code == 200, f"Status: {response.status_code}"
    )
    ...
    if test_plan_id:
        ...
        response = session.put(
            f"{BASE_URL}/admin/plans/{test_plan_id}", json=update_data
        )
        print_test(
            "Update plan",
            response.status_code == 200,
...
print_test("CSV contains data", len(csv_rows) >= 0, f"Rows exported: {len(csv_rows)}")
...
"""
    ...
    if response.status_code == 200:
        ...
        if imported_plans:
            ...
            print_test(
                "Cleanup imported plan",
                response.status_code == 200,
                f"Deleted plan ID: {imported_plan_id}",
            )
...
if response.status_code == 200:
    ...
    print_test(
        "CSV validation catches invalid country_id",
...
if response.status_code == 200:
    result = response.json()
    print_test(
        "CSV validation catches missing required field",
        not result.get("success", True) and len(result.get("errors", [])) > 0,
        f"Errors detected: {len(result.get('errors', []))}",
    )
...
response = session.post(
    f"{BASE_URL}/admin/countries", json={"iso2": "USA", "name": "Test"}
)
print_test(
    "Validation: ISO2 must be 2 characters",
...
if countries_data["countries"] and carriers_data["carriers"]:
    ...
    print_test(
        "Validation: Plan price must be positive",
        response.status_code in [400, 422],
        f"Status: {response.status_code}",
    )
