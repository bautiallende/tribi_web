...
def setup_database():
    """Create test database and override dependencies."""
...
def test_list_countries(client, admin_headers):
    ...
    # Create test data
    client.post("/admin/countries", json={"iso2": "US", "name": "United States"}, headers=admin_headers)
    client.post("/admin/countries", json={"iso2": "MX", "name": "Mexico"}, headers=admin_headers)
    client.post("/admin/countries", json={"iso2": "CA", "name": "Canada"}, headers=admin_headers)
...
def test_list_countries_search(client, admin_headers):
    ...
    client.post("/admin/countries", json={"iso2": "US", "name": "United States"}, headers=admin_headers)
    client.post("/admin/countries", json={"iso2": "GB", "name": "United Kingdom"}, headers=admin_headers)
...
def test_list_plans_filter_country(client, admin_headers):
    ...
    response = client.get(f"/admin/plans?country_id={country1['id']}", headers=admin_headers)
    assert response.status_code == 200
    assert response.json()["total"] == 1
...
def test_update_plan(client, admin_headers):
    ...
    # Create plan
    country = client.post("/admin/countries", json={"iso2": "SE", "name": "Sweden"}, headers=admin_headers).json()
    carrier = client.post("/admin/carriers", json={"name": "Telia"}, headers=admin_headers).json()
...
def test_delete_plan(client, admin_headers):
    ...
    # Create plan
    country = client.post("/admin/countries", json={"iso2": "NO", "name": "Norway"}, headers=admin_headers).json()
    carrier = client.post("/admin/carriers", json={"name": "Telenor"}, headers=admin_headers).json()
