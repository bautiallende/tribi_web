...
class DiagnosticTool:
    ...
    def check_env_var(self, filepath: str, var_name: str, required: bool = True) -> Tuple[bool, str]:
    ...
    def check_process_running(self, pattern: str, name: str) -> bool:
        ...
        try:
            result = subprocess.run(
                ['powershell', '-Command', f'Get-Process | Where-Object {{$_.ProcessName -like "*{pattern}*"}} | Select-Object -First 1'],
                capture_output=True,
                text=True,
                timeout=5
            )
    ...
    def run_diagnostics(self):
        ...
        print(f"{COLORS['CYAN']}Checking system configuration and services...{COLORS['END']}")
        
        # 1. Verificar estructura de archivos
        self.print_header("FILE STRUCTURE")
        base_dir = Path(__file__).parent
        
        self.check_file_exists(str(base_dir / "apps" / "backend" / ".env"), "Backend .env file")
        self.check_file_exists(str(base_dir / "apps" / "backend" / "app" / "main.py"), "Backend main.py")
        self.check_file_exists(str(base_dir / "apps" / "web" / "package.json"), "Web package.json")
        self.check_file_exists(str(base_dir / "apps" / "mobile" / "package.json"), "Mobile package.json")
        self.check_file_exists(str(base_dir / "apps" / "mobile" / "app.config.js"), "Mobile app.config.js")
        ...
        if backend_running:
            ...
            print(f"{COLORS['YELLOW']}Backend not running, skipping API tests{COLORS['END']}")
    ...
    def print_summary(self):
        ...
        if not self.check_port(8000, "Backend"):
            ...
            print(f"    {COLORS['BLUE']}cd apps/backend && python -m uvicorn app.main:app --reload --port 8000{COLORS['END']}")
        
        if not self.check_port(3000, "Frontend"):
            print(f"  • Start the frontend:")
            print(f"    {COLORS['BLUE']}cd apps/web && npm run dev{COLORS['END']}")
        
        if self.issues or self.warnings:
            print(f"\n  • Review the issues above and fix them")
            print(f"  • Check {COLORS['BLUE']}DEBUGGING_GUIDE.md{COLORS['END']} for solutions")
            print(f"  • Run {COLORS['BLUE']}python test_api_complete.py{COLORS['END']} to test API endpoints")
        
        if self.failed == 0:
            print(f"\n{COLORS['GREEN']}{COLORS['BOLD']}✓ ALL CHECKS PASSED!{COLORS['END']}")
            print(f"\n{COLORS['CYAN']}Your system is ready. You can now:{COLORS['END']}")
            print(f"  1. Login at {COLORS['BLUE']}http://localhost:3000/auth{COLORS['END']}")
            print(f"  2. Access admin at {COLORS['BLUE']}http://localhost:3000/admin{COLORS['END']}")
            print(f"  3. Run API tests: {COLORS['BLUE']}python test_api_complete.py{COLORS['END']}")
            return 0
        else:
            print(f"\n{COLORS['RED']}{COLORS['BOLD']}✗ SOME CHECKS FAILED{COLORS['END']}")
            print(f"{COLORS['YELLOW']}Fix the issues above before continuing{COLORS['END']}")
            return 1
